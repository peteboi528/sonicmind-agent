...
class TestValidExternalTrackFiltersCompilation:
    def test_compilation_dropped(self):
        track = ExternalTrack(
            external_id="bv1",
            title="The Weeknd 歌曲合集",
            artist="",
            source="bilibili",
            candidate_kind="compilation",
        )
    ...
    def test_long_mix_dropped(self):
        track = ExternalTrack(
            external_id="yt4",
            title="EDM Non-stop Mix",
            artist="",
            source="youtube",
            candidate_kind="long_mix",
        )
...
class TestRecommendationQualityGate:
    def test_filters_keyword_spam_and_production_assets(self):
        ...
        for index, (title, artist) in enumerate(noisy):
            track = ExternalTrack(external_id=str(index), title=title, artist=artist, source="netease")
    ...
    def test_explicit_type_beat_request_can_bypass_gate(self):
        track = ExternalTrack(external_id="beat-1", title="Drake Type Beat", artist="Producer", source="netease")
...
class TestPlaylistContextGate:
    def test_running_playlist_rejects_sleep_study_and_noise_candidates(self):
        ...
        for index, (title, artist, source) in enumerate(noisy):
            track = ExternalTrack(external_id=f"bad-{index}", title=title, artist=artist, source=source)
    ...
    def test_running_playlist_keeps_normal_song_candidates(self):
        ...
        for index, (title, artist) in enumerate(clean):
            track = ExternalTrack(external_id=f"ok-{index}", title=title, artist=artist, source="netease")
    ...
    def test_non_running_playlist_does_not_apply_running_gate(self):
        track = ExternalTrack(external_id="sleep-1", title="雨声 白噪音", artist="纯音乐馆", source="netease")
