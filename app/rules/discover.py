...
def _local_ratio_from_query(query: str, default: float) -> float:
    ...
    if any(k in q for k in (
        ...
        "少一些本地", "本地少一点", "多用线上", "优先线上", "线上结果多一点",
        "多来点线上", "reduce local", "less local", "fewer local", "prefer online",
    )):
        return 0.15
    return default
...
def _query_matches_track(query: str, track: ExternalTrack) -> bool:
    ...
    entity_hit = any(
        _match_token(entity, part, fuzzy=True)
