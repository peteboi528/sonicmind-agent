...
def test_api_flow():
    ...
    listen = client.post(
        "/listen", json={"user_id": "api-user", "asset_id": asset_id, "duration": 180, "completed": True}
    )
    ...
    track_key = (
        f"{track['source']}:{track['source_id']}"
        if track["source_id"]
        else f"title:{track['title'].lower()}:{track['artist'].lower()}"
    )
    feedback = client.post(
        "/taste/experiment/feedback",
        json={
            "user_id": "api-user",
...
def test_artist_album_tracks_returns_full_ordered_album(monkeypatch):
    ...
    monkeypatch.setattr(
        ns,
        "fetch_netease_album_tracks",
        lambda album_id, limit=100: {
            "id": album_id,
            "name": "Ordered Album",
            "artist": "Artist",
            "cover": "cover",
...
def test_discover_trending_uses_current_billboard_chart(monkeypatch):
    ...
    def fake_detail(playlist_id, limit=8):
        ...
        return {
            ...
            "track_count": 1,
            "tracks": [
                ExternalTrack(external_id=str(playlist_id), title="Current Track", artist="Artist", source="netease")
            ],
        }
...
def test_artist_info_top_albums_use_netease_ids(monkeypatch):
    ...
    monkeypatch.setattr(
        ns,
        "search_netease_artist_albums",
        lambda artist, limit=6: [
...
def test_artist_info_localizes_english_bio_and_keeps_twelve_albums(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda self, artist, limit=12: [{"name": f"Lastfm Album {idx}", "image": ""} for idx in range(limit)],
    )
    monkeypatch.setattr(
        ns,
        "search_netease_artist_albums",
        lambda artist, limit=12: [
