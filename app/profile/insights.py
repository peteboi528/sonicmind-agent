...
def generate_profile_insights(profile: UserProfileResponse) -> list[ProfileInsight]:
    ...
    def add(dimension: str, key: str, title: str, explanation: str, confidence: float, evidence: list[str]) -> None:
        insights.append(
            ProfileInsight(
    ...
    for scene in profile.scenes[:3]:
        add(
            ...
            evidence=[f"偏好风格：{('、'.join(scene.preferred_genres) or '—')}", scene.recommendation_strategy],
        )
...
def apply_feedback(
    insights: list[ProfileInsight],
    feedback: ProfileFeedbackState | None,
) -> list[ProfileInsight]:
